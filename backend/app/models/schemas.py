...
class TripRequest(BaseModel):
    ...
    # ---- 进阶级新增字段 ----
    budget_limit: Optional[int] = Field(default=None, description="预算上限(元)", example=3000)
    cities: Optional[List[str]] = Field(default=None, description="多城市联游列表(优先于city字段)", example=["北京", "西安", "成都"])
...
class Attraction(BaseModel):
    ...
    opening_hours: Optional[str] = Field(default=None, description="开放时间（高德POI实时数据）")
...
class DayPlan(BaseModel):
    ...
    def parse_accommodation(cls, v):
        """兼容LLM返回dict/None的情况，提取酒店名称或直接stringify"""
        if v is None:
            return ""
...
class WeatherInfo(BaseModel):
    ...
    weather_warning: Optional[str] = Field(default=None, description="天气预警（极端天气时）")
...
class GuideAskRequest(BaseModel):
    ...
    top_k: int = Field(default=4, ge=1, le=8, description="检索文档数量")


class GuideReference(BaseModel):
    """导游问答引用资料"""
    title: str = Field(default="", description="资料标题")
    city: str = Field(default="", description="资料所属城市")
    attraction_name: str = Field(default="", description="关联景点")
    snippet: str = Field(default="", description="资料摘要片段")
    source: str = Field(default="knowledge_base", description="资料来源")
...
class ShareCreateResponse(BaseModel):
    """行程分享创建响应"""
    success: bool
    share_id: str = Field(default="", description="8位分享 Token")
    share_url: str = Field(default="", description="完整分享 URL")
    expires_at: str = Field(default="", description="过期时间 ISO 格式")
